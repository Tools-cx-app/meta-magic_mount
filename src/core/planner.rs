...
// Helper function: Check if a directory recursively contains any actual files
// (including symlinks, devices, etc.). Returns true if content is found.
fn has_files_recursive(path: &Path) -> bool {
    if let Ok(entries) = fs::read_dir(path) {
        for entry in entries.flatten() {
            ...
            // If it is a directory, check recursively.
            // If it is a file, symlink, or other type, it counts as content.
            if file_type.is_dir() {
